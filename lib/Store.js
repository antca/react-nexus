...
class State {
    ...
    return (props, propName, componentName, ...rest) => {
      ...
      if (!(state instanceof State)) {
        return new TypeError(
          `Expecting prop ${propName} of ${componentName} to be an instance of Store.State.`
        );
      }
      if (!state.isResolved()) {
        return null;
  ...
  static propType(type) {
    ...
    const propType = (props, propName, ...rest) => {
      const state = props[propName];
      if (state !== void 0) {
        return validatePropType(props, propName, ...rest);
      }
    };
   ...
   */
  dump() {
    return _.pick(this, 'status', 'reason', 'value', 'meta');
  }
}
...
class Store extends Routable {
   ...
   */
  constructor(route, { dumpState, fetch, loadState, observe, readFromState }) {
    super(route);
