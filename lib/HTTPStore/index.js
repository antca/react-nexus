...
class HTTPStore extends Store {
  ...
  constructor(route, httpConfig, options = {}) {
    ...
    const mapHTTPStoreKeys = _map(keys, (key) => [
      key,
      (...args) => Reflect.apply(HTTPStore.prototype[key], this, args),
    ]);
    super(route, _fromPairs(mapHTTPStoreKeys));
  ...
  async _forceFetch(path, pathQuery) {
    ...
    const { pathname, query } = url.parse(
      rewritePath(pathQuery, path),
      true,
      true
    );
    const uri = url.format(
      Object.assign({}, this.httpConfig, { pathname, query })
    );
    ...
    if (!res.ok) {
      return this._set(
        path,
        Store.State.reject(currentState.value, await res.text(), { path })
      );
    }
