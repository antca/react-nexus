...
class HTTPStore extends Store {
  ...
  constructor(route, httpConfig, options = {}) {
    ...
    this.data = new Map();
    this._options = Object.assign({ rewritePath: (query, path) => path }, options);
  }
  ...
  async _fetch(path, query) {
    ...
    const options = this._getOptions(path);
    const state = await this._forceFetch(options.rewritePath(query, path), options);
  ...
  /**
   * Returns an options object build with the global store options and the path specific options if path is defined.
   * @param  {String} [path] Path of the path specific options.
   * @return {Object} Options object.
   * @param {Object} query Query that will be use to fetch the store and set the state.
   */
  _getOptions(path) {
    return Object.assign({}, this._options, path ? this._findOrCreatePathData(path).options : {});
  }

  /**
   * Updates options
   * @param  {Object|null} [query] Query of the query specific options.
