 ...
 * @return {Promise} Promise for a simplified react elements tree which will produce
 * the same output as the original tree when rendered as html, but much faster.
 */
async function prepare(element, context = {}) {
  ...
  if(children === null && typeof element.type !== 'string') {
    return React.createElement(() => null);
  }
  const simplifiedChildren = await prepare(children, childContext);
  if(typeof element.type === 'string') {
    const filteredProps = omit(element.props, 'children');
    if(element.type !== 'input' && element.type !== 'textarea') {
      Object.assign(filteredProps, { children: simplifiedChildren });
    }
