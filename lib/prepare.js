...
function render(inst, context = {}) {
  return [
    inst.render(),
    inst.getChildContext ? inst.getChildContext() : context,
  ];
}
...
async function prepareElement(element, context) {
  ...
  const { type, props } = element;
  // Native element or fragment
  if (typeof type === 'string' || type === Symbol.for('react.fragment')) {
...
async function prepare(element, context = {}) {
  ...
  if (Array.isArray(element)) {
    return Promise.all(element.map((children) => prepare(children, context)));
  }
  ...
  if (typeof element.type === 'string') {
    ...
    const filteredProps = _omit(element.props, 'children');
