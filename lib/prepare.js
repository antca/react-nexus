...
async function prepare(element, context = {}) {
  // When prepare is called with an array of elements, pass them through prepare to manage then individually.
  if(Array.isArray(element)) {
    return Promise.map(element, (children) => prepare(children, context));
  }
  // If the element is null or a primitive, nothing special to do.
  if(element === null || typeof element !== 'object') {
    return element;
  }
  // Satisfy and then get the childrens of the element.
  const [children, childContext] = await prepareElement(element, context);
  // Rerecurcively prepare the children.
  ...
  // If the element is not native but it's child is null or false,
