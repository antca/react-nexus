...
class MemoryStore extends Store {
  ...
  /**
   * Retrieves the value of the current state of the Store.
   *  - Returns the value of the store if the status of the state is resolved.
   *  - Throws a generic error with the reason if the status of the state is rejected.
   *  - Returns null if the state of the store is pending or if there was no corresponding path.
   * @throws Error
   * @param  {Object} query Query used to get the state of the store
   * @return {MemoryStore} Instance of the current MemoryStore.
   */
  get(query) {
    const state = this.readFromState(query);
    if(state.isResolved()) {
      return state.value;
    }
    if(state.isRejected()) {
