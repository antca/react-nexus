...
class MemoryStore extends Store {
  ...
  constructor(route) {
    ...
    const mapMemoryStoreKeys = _map(keys, (key) => [
      key,
      (...args) => Reflect.apply(MemoryStore.prototype[key], this, args),
    ]);
    super(route, _fromPairs(mapMemoryStoreKeys));
    this.data = new Map();
  }
   ...
   */
  loadState(state) {
    const entriesKeys = _keys(entriesToObject(this.data.entries()));
    _each(entriesKeys, (path) => {
      if (!state.hasOwnProperty(path)) {
        this._delete(path);
      }
    });
    _each(state, (dump, path) => this._set(path, Store.State.create(dump)));
    return this;
  }
   ...
   */
  dumpState() {
    return _mapValues(entriesToObject(this.data.entries()), ({ state }) =>
      state.dump()
    );
  }
  ...
  _findOrCreatePathData(path) {
    if (!this.data.has(path)) {
      this.data.set(path, {
        observers: new Set(),
        state: Store.State.reject(void 0, `Path ${path} is not initialized!`, {
          path,
          uninitialized: true,
        }),
      });
  ...
  set(query, valueOrState) {
    const path = this.toPath(query);
    const state =
      valueOrState instanceof Store.State
        ? valueOrState
        : Store.State.resolve(valueOrState, { path });
  ...
  _delete(path) {
    ...
    const state = Store.State.reject(data.store.value, 'Store deleted', {
      path,
    });
