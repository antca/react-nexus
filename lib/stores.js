...
function stores(
  ...
  return (Component) =>
    preparable(async function prepare(props, context) {
      ...
      const deps = _map(bindings, (binding) => {
        const state = flux.readStoreFromState(binding);
        if (
          state.isResolved() ||
          (state.isRejected() && !state.meta.uninitialized)
        ) {
      ...
      class StoresComponent extends React.Component {
        ...
        constructor(props, context) {
          ...
          const flux = context[fluxKey];
          const bindings = getBindings(props, flux);
          this.state = _mapValues(bindings, (binding) =>
            flux.readStoreFromState(binding)
          );
        ...
        componentDidMount() {
          ...
          const flux = context[fluxKey];
          const bindings = getBindings(props, flux);
          _each(bindings, (binding, key) => {
            this.updateStoreState(key, flux.readStoreFromState(binding));
            this.observers.set(
              key,
              flux.observeStore(binding, (state) =>
                this.updateStoreState(key, state)
              )
            );
          });
        }
        ...
        UNSAFE_componentWillReceiveProps(nextProps) {
          ...
          const [removed, added] = diff(nextBindings, prevBindings, _isEqual);
          const removedKeys = _keys(removed);
          _each(removedKeys, (key) => {
            this.updateStoreState(key, void 0);
            this.observers.get(key)();
            this.observers.delete(key);
          });
          _each(added, (binding, key) => {
            this.updateStoreState(key, flux.readStoreFromState(binding));
            this.observers.set(
              key,
              flux.observeStore(binding, (state) =>
