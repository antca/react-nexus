...
async function request(httpConfig, method, pathname, data) {
  const uri = url.format(Object.assign({}, httpConfig, { pathname }));
...
function createFlux({ port }) {
  ...
  return Flux.create({
    actions: [
      action(
        '/users/create',
        async function createUser(flux, query, { userName, rank }) {
          await request(httpConfig, 'POST', `/users/create`, {
            userName,
            rank,
          });
      ...
      action(
        '/users/:userId/delete',
        async function deleteUser(flux, { userId }) {
      ...
      action(
        '/users/:userId/update',
        async function updateUser(flux, query, { userName, rank }) {
          await request(httpConfig, 'POST', `/users/${query.userId}/update`, {
            userName,
            rank,
          });
          await flux.store('/users').fetch({});
      ...
      action(
        '/ui/users/toggle/visibility',
        async function toggleVisibilityUsers(flux) {
          const previousValue = await flux
            .store('/ui/users/visibility')
            .fetch({});
