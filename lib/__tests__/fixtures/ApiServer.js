    ...
    this.app.use(cors({ origin: '*' })).use(
      createRouter()
        ...
        .get('/users/:userId', async function $user(ctx) {
          ...
          ctx.body = _find(
            users,
            ({ userId }) => userId === parseInt(userIdToFind)
          );
          ctx.status = 200;
        })
        ...
        .post('/users/create', createBody(), async function $createUser(ctx) {
          ...
          users.push({ userId: _last(users).userId + 1, userName, rank });
          ctx.status = 200;
        })
        .post(
          '/users/:userId/update',
        ...
        .delete('/users/:userId/delete', async function $deleteUser(ctx) {
          ...
          users = users.filter(
            ({ userId }) => userId !== parseInt(userIdToRemove)
          );
          ctx.status = 200;
        })
        .routes()
