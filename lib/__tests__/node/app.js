...
describe('@root', () => {
  ...
  it('prepare App on server', async function test() {
    ...
    await prepare(app);
    should(ReactDOMServer.renderToStaticMarkup(app)).be.deepEqual(
      ReactDOMServer.renderToStaticMarkup(
        <div className="App">
          <div>
            <div>
              <button id="UsersVisibility">{'Show/Hide Users'}</button>
            </div>
            <ul className="Users">
              <li>
                <div className="User">
  ...
  it('dispatches Actions', async function test() {
    ...
    await flux.dispatchAction('/users/create', {
      userName: 'Marcel',
      rank: 'Bronze',
    });
    await flux.dispatchAction('/users/2/update', {
      userName: 'Carol',
      rank: 'Diamond',
    });
    const app = <App flux={flux} />;
