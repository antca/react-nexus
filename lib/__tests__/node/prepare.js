...
describe('prepare', () => {
  it('resolves to an identical context on DOM elements', () => {
    ...
    return prepare(<div />, context).then(() =>
      should(context).be.deepEqual({ bar: 'foo' })
    );
  });
  ...
  it('prepares a simple element with context side effects', () => {
    ...
    const context = {};
    return prepare(<Test bar="foo" />, context).then(() =>
      should(context).have.property('bar').which.is.exactly('foo')
    );
  });
  it('prepares nested elements', () => {
    ...
    return prepare(
      ...
      should(spy)
        .have.property('callCount')
        .which.is.exactly(expectedNumberOfCalls)
    );
  });
  ...
  it('prepares stateless components correctly', async function test() {
    ...
    StatelessComponent[preparable.$prepare] = async function preps(
      props,
      context
    ) {
    ...
    const context = {};
    return await prepare(<StatelessComponent />, context).then(() =>
      should(context).have.property('bar').which.is.exactly('foo')
    );
  });
  it('prepares components using fragments correctly', async function test() {
    const ComponentWithFragment = () => <>{null}</>;
    ComponentWithFragment[preparable.$prepare] = async function preps(
      props,
      context
    ) {
      context.bar = await Promise.resolve('foo');
    ...
    const context = {};
    return await prepare(<ComponentWithFragment />, context).then(() =>
      should(context).have.property('bar').which.is.exactly('foo')
    );
  });
  it('doesn’t fetch the same HTTPStore more than once', async function test() {
    ...
    const apiMock = nock(url.format(testHttpConf))
      .get(testStorePath)
      .reply(HTTPStatus.OK, { message: 'Hello world' });
    ...
    const getStoreDeps = _constant({ testStore: testStorePath });
    ...
    should(apiMock.isDone()).be.exactly(
      true,
  ...
  describe('preparedApp', () => {
    it('is rendered as the original app', () => {
      ...
      const app = <Foo />;
      return prepare(app).then((preparedApp) => {
