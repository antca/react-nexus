...
describe('stores', () => {
  ...
  it('should set state to undefined when binding is removed', () => {
    ...
    act(() => {
      ReactDOM.render(<DecoratedBar bindFoo flux={flux} />, document.body);
    });

    should(document.body.innerHTML).be.exactly('<span>foo is not undefined</span>');

    act(() => {
      ReactDOM.render(<DecoratedBar bindFoo={false} flux={flux} />, document.body);
    });

    should(document.body.innerHTML).be.exactly('<span>foo is undefined</span>');
  });
});
