...
function createMatcher({ keys, re }) {
  return function matchPath(path) {
    ...
    const mapMatchingKeys = _map(keys, ({ name }, i) => [name, m[i + 1]]);
    return _fromPairs(mapMatchingKeys);
  };
}
...
function findInMatchers(path, collection) {
  const mapMatchers = _map(collection, ([matcher, routable]) => [
    routable,
    matcher(path),
  ]);
  return _find(mapMatchers, ([, matchPath]) => matchPath !== null);
}
...
class Flux {
  ...
  findAction(routeToFind) {
    const filteredActions = _filter(
      this.actions,
      ([, { route }]) => route === routeToFind
    );
    const mapActions = _map(filteredActions, ([, action]) => action);
    return _first(mapActions);
  }
  ...
  findStore(routeToFind) {
    const filteredStores = _filter(
      this.stores,
      ([, { route }]) => route === routeToFind
    );
    const mapStores = _map(filteredStores, ([, store]) => store);
    return _first(mapStores);
  }
